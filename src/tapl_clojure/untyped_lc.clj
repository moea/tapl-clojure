...
(defn term-shift-above [d c t]
  (term-walk
   (fn [c x n] [:term/var (cond-> x (<= c x) (+ d)) (+ n d)])
   c t))
...
(defn term-sub [j s t]
  (term-walk
   (fn [c x n] (if (= x (+ j c)) (term-shift c s) [:term/var x n]))
   0 t))
...
(defn eval1 [ctx t]
  (match t
    ([:term/app [:term/abs _ t12] v] :guard #(is-val? (last %)))   (term-sub-top v t12)
    ([:term/app _ _]                 :guard #(is-val? (second %))) (update t 2 (partial eval1 ctx))
